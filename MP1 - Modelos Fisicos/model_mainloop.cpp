...
int main ()
{
  ...
  while (1) {
    ...
    for (int j=0; j<corpos->size(); j++)
    {
      ...
      std::cout << (*corpos)[j]->get_aceleracao() << "- Forca = ";//edit: Add imprime aceleracao
      std::cout << (*corpos)[j]->get_forca() << std::endl;//edit: Add imprime forca
    }
